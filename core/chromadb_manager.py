...
class ChromaDBManager:
    def __init__(self, path: str = None):
        ...
        if self.client and self.hpoa and self.ont_hp is not None:
            logger.info(f"ChromaDB client has been created and {self.hpoa} and {self.ont_hp} are available")
            self.hp_embeddings = self.create_hpo_id_to_embedding(self.ont_hp)
            logger.info(f"hp_to_embeddings dict has been created")
            self.disease_to_hps = self.create_disease_to_hps_dict(self.hpoa)
